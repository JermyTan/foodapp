      ...
      if (result.rows.length !== 0) {
        ...
        db.query(createManagerQuery, async (err2, result2) => {
          if (err2) {
            ...
            response.status(500).json({ success: false, msg: 'Failed to create manager account.' });
          } else {
            console.log('Result', result2[2].rows, result2[3].rows);
            if (result2[2].rows.id == result2[3].rows.id)
              response.status(200).json({ success: true, msg: 'Created user/manager with id' });
            else {
              response.status(404).json({ success: false, msg: `Failed to create manager.` });
            }
          }
        });
      }
