...
exports.createCustomer = async (req, response) => {
  ...
  const createCustomerQuery = `BEGIN;
...
exports.updateCustomerCard = async (req, response) => {
  ...
  const rows = await db.query(
    "UPDATE Customers SET cardnum = NULLIF($1, 0) WHERE id = $2 returning *",
    [cardnum, id],
    (err, result) => {
      if (err) {
        console.error(err.stack);
        ...
        if (!result.rows[0]) {
          response
            .status(404)
            .json(`Failed to update cardnum for customer ${id}`);
        } else {
          console.log(`Successfully updated customer ${id} card number`);
          response
            .status(200)
            .json({ msg: `Successfully updated customer ${id} card number` });
        }
      }
    }
  );
...
exports.getCustomerOrders = async (req, response) => {
  ...
  const getCustomerOrdersQuery = `SELECT json_build_object(
    'oid', oid,
    'fprice', fprice,
    'location', location,
    'dfee', dfee,
  ...
  const rows = await db.query(addReviewQuery, (err, result) => {
    if (err) {
      ...
      if (!result.rows[0]) {
        ...
        response
          .status(200)
          .json({ msg: `Successfully added/updated review for order ${oid}` });
      }
    }
  });
};
  ...
  const rows = await db.query(addRatingQuery, (err, result) => {
    if (err) {
      ...
      if (!result.rows[0]) {
        ...
        response
          .status(200)
          .json({ msg: `Successfully added/updated rating for order ${oid}` });
      }
    }
  });
};
  ...
  db.query(getRecentLocationsQuery, (err, result) => {
    if (err) {
      ...
      if (!result.rows[0]) {
        response
          .status(404)
          .json(`Failed to retrieve recent delivery locations`);
      } else {
